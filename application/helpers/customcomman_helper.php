...
//                        $query_multilang = "SELECT GROUP_CONCAT(slug_url) AS slug
//                                        FROM cms WHERE status = 1";
//                        $result_multilang = mysql_query($query_multilang);
//                        $row_multilang = mysql_fetch_assoc($result_multilang);
...
//                        echo $row_multilang[0]->slug; exit;
                ...
                // For Admin Menu
                    ...
                    // echo '<pre>';print_r($menuData['items']); exit;
                    // echo base_url(uri_string());exit; // get Full URL

                    $seluri = $ci->uri->segment(2);
                    $link = implode('/', array_slice(explode('/', $menuData['items'][$itemId]['m']['link']), 1, 1));

//                    if (!empty($menuData['items'][$itemId]['m']['parent_id']) && ($seluri === $link)) {
                    // if (!empty($menuData['items'][$itemId]['m']['parent_id'])) {

                    if (isset($menuData['parents'][$itemId]) && ($seluri === $link)) {

                        //print_r($menuData['items'][$itemId]['m']); exit;
                ...
                if (isset($menuData['parents'][$itemId])) {
                    ...
                    $html .= '<li id="menu' . $menuData['items'][$itemId]['m']['id'] . '"><a href="' . site_url() . $menulink . '" class="haschild ' . $selected . '" >' . $menuData['items'][$itemId]['m']['title'] . "</a>";
                } else {
...
//                    echo '<pre>';
//                    echo $sess_var = ($ci->session->userdata['front']['user_id']);
//                    exit;
//                    if ($menuData['items'][$itemId]['m']['id'] != 271) {
//                        $html .= '<li id="menu' . $menuData['items'][$itemId]['m']['id'] . '"><a href="' . site_url() . $menulink . '" class="' . $selected . '">' . $menuData['items'][$itemId]['m']['title'] . "</a>";
//                    }
//
...
//                    }
                    ...
                    if ($menuData['items'][$itemId]['m']['id'] != 271) {
                        $html .= '<li id="menu' . $menuData['items'][$itemId]['m']['id'] . '"><a href="' . site_base_url() . $menulink . '" class="' . $selected . '">' . $menuData['items'][$itemId]['m']['title'] . "</a>";
                    } else if (!empty($ci->session->userdata['front']['user_id']) && ($menuData['items'][$itemId]['m']['id'] == 271)) {
