...
class States_model extends Base_Model {
    ...
    function get_record_listing() {
        ...
        $this->db->select('R.*');
        $this->db->from($this->_tbl_states . ' AS R');
        $query = $this->db->get();
    ...
    public function save_record($data) {
        ...
        if ($id != 0 && $id != "") {
            $this->db->where('id', $id);
            $this->db->update($this->_tbl_states, $data);
            $id = $id;
        } else {
            $this->db->insert($this->_tbl_states, $data);
    ...
    public function delete_record($id) {
        ...
        $this->db->where('id', $id);
        return $this->db->delete($this->_tbl_states);
    }

    function get_record_by_state_name($name = '') {
        if ($name != '') {
            $this->db->select('*');
            $this->db->from($this->_tbl_states);
