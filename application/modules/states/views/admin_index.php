    ...
    <div class="main-container">
        ...
        <div class="grid-data grid-data-table">
            <div class="add-new">
                ...
                <?php } ?>
                <?php echo anchor(site_url() . 'admin/states/action/add', lang('add-record'), 'title="Add Record" style="text-align:center;width:100%;"'); ?>
            </div>
            <?php 
                    ...
                    <tbody bgcolor="#fff">
                        <tr>
                            ...
                            <th width="30px"><?php echo lang('no') ?></th><th>
                                ...
                                ?>
                                <a href="#" onclick="sort_data('name', '<?php echo $field_sort_order; ?>');" >
                                    State Name
                                    <?php
                                    if ($sort_by == 'name')
                                    {
                                ...
                                $field_sort_order = 'asc';
                                $sort_image = 'srt_down.png';
                                if ($sort_by == 'tax' && $sort_order == 'asc')
                                {
                                    $sort_image = 'srt_up.png';
                                    $field_sort_order = 'desc';
                                }
                                ?>
                                <a href="#" onclick="sort_data('tax', '<?php echo $field_sort_order; ?>');" >
                                    Tax
                                    <?php
                                    if ($sort_by == 'tax')
                                    {
                        ...
                        foreach ($records as $record)
                        {
                            ...
                            <tr class="<?php echo $class; ?> rows" >
                                ...
                                <td><?php echo $i; ?></td><td><?php echo $record['R']['name']; ?></td><td><?php echo $record['R']['tax']; ?> %</td>
                                <td>
                                    <div class="action">
                                        <?php $record_id = $record['R']['id']; ?>
                                        <div class="edit"><a href="<?php echo site_url(); ?>admin/states/action/edit/<?php echo $record_id ?>" title="<?php echo lang('edit') ?>"><?php echo add_image(array('edit.png')); ?></a></div>
        ...
        $options = array(
            ...
            'isAjaxRequest' => 1,
            'base_url' => base_url() . "admin/states/index",
            'params' => $querystr,
            'element' => 'ajax_table'
        );
...
<script type="text/javascript">	    
    ...
    function delete_record(id){        
        ...
        if(res){
            $.ajax({
                type:'POST',
                url:'<?php echo base_url(); ?>admin/states/delete',
                ...
                success: function(data) {
                    ...
                    ajaxLink('<?php echo base_url(); ?>admin/states/index','ajax_table','<?php echo $querystr; ?>'+pageno);
    ...
    {            
        ...
        $.ajax({
            type:'POST',
            url:'<?php echo base_url(); ?>admin/states/index',
