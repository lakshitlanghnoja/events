 ...
 * @author panks
 */
class plants_model extends Base_Model
{

    protected $_tbl_plants = TBL_PLANTS;
    ...
    public function save_plant($data)
    {
        ...
        if(isset($data['botanicalname']))
        {
            $user_data ['botanicalname'] = $data['botanicalname'];
        }
        if(isset($data['commonname']))
        {
            $user_data['commonname'] = $data['commonname'];
        }
        ...
        if(isset($user_data['id']) &&  $user_data['id']!= 0 && $user_data['id'] != "")
        {
            ...
            $this->db->update($this->_tbl_plants, $user_data);
            $id = $user_data['id'];
        }
        else
        {
            ...
            if($this->db->insert($this->_tbl_plants, $data))
            {
     ...
     * @param integer $id
     */
    function get_plant_detail($id = 0)
    {
        //Type Casting
        $id = intval($id);
        ...
        $tableusers = $this->db->get($this->_tbl_plants);
...
//        exit;
        ...
        if($this->search_term != "")
        {
            $this->db->like("LOWER(u.botanicalname)", strtolower($this->search_term));
        }
        ...
        $this->db->select('u.*');
        $this->db->from($this->_tbl_plants . ' AS u');
    ...
    public function delete_plant($id)
    {
        ...
        return $this->db->update($this->_tbl_plants);
    }
    ...
    public function inactive_records($id = array())
    {
        ...
        $this->db->update($this->_tbl_plants);
